...
func TestRunRefreshWritesFreshnessSummary(t *testing.T) {
	root := t.TempDir()
	repo := filepath.Join(root, "service")
	runGit(t, root, "init", "-b", "main", repo)
	runGit(t, repo, "config", "user.email", "test@example.com")
	...
	var output bytes.Buffer
	if err := Run([]string{"refresh", root}, &output); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(output.String(), `"index_state":"up_to_date"`) {
		t.Fatalf("refresh output = %s", output.String())
	}
}
